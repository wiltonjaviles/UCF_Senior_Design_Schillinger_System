    ...
    <div>
      <Container>
        ...
        <TextToSpeech 
          ...
          A ﬁgure such as 2 +1 +1 may be conceived as one of the elementary rhythmic patterns in 4 4 time. Yet it is possible, with this method of analysis, to assign it directly to a deﬁnite place in a deﬁnite resultant—the second bar of 
          The longer patterns, such as the resultants produced by higher number-values or by more than two generators, possess enough variation in themselves.
          Musical memory does not emphasize a group of 20 or more bars as one indivisible pattern. Therefore, the recurrence of such pattern seems to be less monotonous than the recurrence of a short pattern. Short patterns obviously call for variations. There are many outstanding compositions in which direct recurrence of a short pattern is used throughout the entire composition—for example, the ﬁrst movement of Beethoven’s symphony No. 5; Chopin’s waltz
          No. 7, the second theme. In such compositions, rhythmic monotony is usually compensated for by the variety of devices used on some other components— it may be the dynamic, the harmonic, or the melodic composition of a piece that makes this music sound interesting. The best method by which to detect the effect of the purely rhythmic patterns is to isolate them from all other components, i.e., to take a fragment of a composition, Or the entire composition,
          ...
          Binomial 1 + 1
          Binomial 5 + 3
          The additional component may emphasize the entire duration of the accented attack, as in the previous example, or be considerably shorter (just to
          single out the moment of attack).
          Example:
          Binomial 2 + 1
          Variations of rests may be combined with variations of the previous com-
          ponents.
          (4) Split-unit groups
          ...
          (a) Clockwise circular permutations:
          3 permutations
          ...
          + (1+4+3+2) + (1+4+2+3) + (1+2+4+3) + (2+1+4+3) +
          + (1+3+4+2) + (1+3+2+4) + (1+2+3+4) + (2+1+3+4) +
          + (3+1+4+2) + (3+1+2+4) + (3+2+1+4) + (2+3+1+4)
          ...
          + (2+1+2+3) + (1+2+3+2) + (2+2+3+1) + (2+3+1+2)
          ...
          a = z b = 1
          ( (circled 1) +1+1+1) + (1+ (circled 1) +1+1) + (1+1+ (circled 1) +1) + (1+1+1+ (circled 1) )
          (1+ (circled 1) +1+1) +(1+1+ (circled 1) +1)+(1+1+1+ (circled 1) ) +( (circled 1) +1+1+1)
          (1+1+ (circled 1) +1)+(1+1+1+ (circled 1) ) +( (circled 1) +1+1+1) +(1+ (circled 1) +1+1)
          (1+1+1+ (circled 1) )+( (circled 1) +1+1+1)+(1+ (circled 1) +1+1)+(1+1+ (circled 1) +1)
          ...
          Example: r underlined 5%4= 4+ 1 +3 +2
